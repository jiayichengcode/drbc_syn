    ...
    for num, i in enumerate(range(len(rebalancing_dates))):
        ...
        if i > 0:
            ...
            if len(current_perms) > 0:
                ...
                # add new column for risk free asset, annual return is r, monthly return is (1 + r)^(1/12) - 1
                monthly_returns_oos[0] = (1 + r)**(1/12) - 1
        ...
        if len(eligible_stocks) < n_stocks:
            ...
            monthly_returns_train[0] = (1 + r)**(1/12) - 1 + np.random.normal(0, 0.01, len(monthly_returns_train))  #risk free add
            # add risk free permno 0 to selected_perms
            selected_perms = np.append(selected_perms, 0)
            if monthly_returns_train.shape[0] < 118:
                new_weights = np.ones(n_stocks+1) / (n_stocks+1) # risk free add
            else:
...
def run_single_backtest_with_preselected_stocks(
    all_data_df: pd.DataFrame, 
    preselected_stocks: list,
    transaction_cost: float,
    annual_target_return: float,
    backtest_start_date: str,
    backtest_end_date: str, 
    ...
    """
    Runs a single backtest for the DRMV portfolio strategy with pre-selected stocks.

    Args:
        all_data_df (pd.DataFrame): DataFrame containing all stock data.
        preselected_stocks (list): A list of lists, where each inner list contains the 'permno's 
                                   of stocks to be included in the portfolio for each rebalancing period.
                                   The outer list's length must match the number of rebalancing periods.
        transaction_cost (float): The transaction cost rate.
        annual_target_return (float): The target annual return for the portfolio.
        backtest_start_date (str): The start date for the backtest (e.g., 'YYYY-MM-DD').
        backtest_end_date (str): The end date for the backtest (e.g., 'YYYY-MM-DD').
        r (float, optional): The annual risk-free rate. Defaults to 0.02.

    Returns:
        pd.Series: A Series containing the wealth history of the portfolio over the backtest period.
    """
    rebalancing_dates = pd.date_range(start=backtest_start_date, end=backtest_end_date, freq='MS')

    if len(preselected_stocks) != len(rebalancing_dates):
        raise ValueError("The length of 'preselected_stocks' must be equal to the number of rebalancing periods.")
    ...
    for i, rebal_date in enumerate(rebalancing_dates):
        ...
        # Use pre-selected stocks for the current rebalancing period
        selected_perms = preselected_stocks[i]

        if not selected_perms:
            new_weights = np.array([])
        else:
            selected_perms = np.array(selected_perms)
            n_stocks = len(selected_perms)
            training_start_date = rebal_date - relativedelta(years=10)
            training_data = all_data_df[(all_data_df['permno'].isin(selected_perms)) & (all_data_df['date'] >= training_start_date) & (all_data_df['date'] < rebal_date)]
            monthly_returns_train = training_data.pivot_table(index='date', columns='permno', values='ret').resample('M').apply(lambda x: (1 + x).prod() - 1).dropna()
            monthly_returns_train[0] = (1 + r)**(1/12) - 1 + np.random.normal(0, 0.03, len(monthly_returns_train))  #risk free add
            ...
            if monthly_returns_train.shape[0] < 118:
                ...
                # Ensure columns are in the correct order before calculating weights
                monthly_returns_train = monthly_returns_train.reindex(columns=selected_perms, fill_value=0)
                new_weights = get_drmv_weights(monthly_returns_train, annual_target_return)
                print(new_weights)
...
if __name__ == "__main__":
    ...
    if not all_wealth_curves: print("No wealth curves were generated.")
        ...
        results_df.to_csv(f'res/drmv_backtest_results_riskfree_{seed}_n{N_STOCKS}_1524.csv', index=True)
        print(f"Results saved to 'res/drmv_backtest_results_riskfree_{seed}_n{N_STOCKS}_1524.csv'.")